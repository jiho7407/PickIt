...
- `supabase/migrations/{timestamp}_create_notification_candidate_rpc.sql` (create)
...
- 조건: `dilemmas.followup_due_at <= now_ts` AND 상태가 `open|decided|followup_due` AND 해당 dilemma의 followups row 없음.
- 반환: dilemma 요약 + due 경과 일수.

### RPC: `get_notification_candidates()`
...
5. permission: 비운영자가 RPC 호출 시 차단(운영자 role 정책 사용 시).
