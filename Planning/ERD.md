...
`followup_due_at <= now`이고 아직 followup이 없는 고민을 반환한다.
...
- update/delete: author only, or disallow in MVP
...
- `profiles.role = 'operator'`인 사용자만 운영자 후보 조회 RPC를 호출할 수 있다.
